...
class FTBartEncoderWeight(object):
    ...
    def load_from_model(self, model):
        ...
        for name, param in model.named_parameters():
            ...
            if name.find("encoder.layers") != -1 or name.find("encoder.layernorm_embedding") != -1 or name.find(
...
class FTBartEncoder(nn.Module):
    def __init__(self, encoder_weight_list, lib_path, head_num, head_size, inter_size, d_model, is_remove_padding,
                 num_layer, num_bucket=32, max_distance=128, sparse=False, q_scaling=1.0, tensor_para_size=1,
                 pipeline_para_size=1,
                 bart_with_bias=True, mbart=False, position_embedding_type=1, activation_type="gelu",
        ...
        try:
            self.encoder = torch.classes.FasterTransformer.BartEncoder(*encoder_weight_list, head_num, head_size,
                                                                       inter_size, d_model,
                                                                       is_remove_padding, num_layer, num_bucket,
                                                                       max_distance, sparse, q_scaling,
                                                                       tensor_para_size, pipeline_para_size,
                                                                       bart_with_bias, mbart, position_embedding_type,
                                                                       activation_type, layernorm_type)
        except:
            self.encoder = torch.classes.FasterTransformerBartEncoder(*encoder_weight_list, head_num, head_size,
                                                                      inter_size, d_model,
                                                                      is_remove_padding, num_layer, num_bucket,
                                                                      max_distance, sparse, q_scaling, tensor_para_size,
                                                                      pipeline_para_size,
                                                                      bart_with_bias, mbart, position_embedding_type,
                                                                      activation_type, layernorm_type)
