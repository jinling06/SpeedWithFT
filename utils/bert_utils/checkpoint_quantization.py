...
def checkpoint_quantization(init_dict, sparse, ths_path='./lib/libth_transformer.so', verbose=True):
    ...
    def init_graph():
        ...
        for name, tensor_value in init_dict.items():
            if "intermediate.dense.weight" in name and amaxTotalNum == 0:
                amaxTotalNum = ACTIVATION_AMAX_NUM + 9 * tensor_value.size(
                    1) + INT8O_GEMM_NUM + TRT_FUSED_MHA_AMAX_NUM + SCALE_RESERVE_NUM
                if verbose:
    ...
    for i in range(layer_num):
            ...
            if amax_name in int8O_gemm_input_list:
                ...
                if amax_name == "attention.self.query._input_quantizer":
                    int8O_gemm_input_amax_list[
                        int8O_gemm_input_list.index("attention.self.key._input_quantizer")] = amax
                    int8O_gemm_input_amax_list[
                        int8O_gemm_input_list.index("attention.self.value._input_quantizer")] = amax
        ...
        for kernel_id, kernel_name in enumerate(kernel_name_list):
            ...
            if (amax2.dim() == 0):
                quant_max_processed = torch.full((kernel.size(1),), amax2.item(), dtype=amax2.dtype,
                                                 device=amax2.device)
            else:
        ...
        for j in range(INT8O_GEMM_NUM):
            amaxList[amax_id] = (int8O_gemm_input_amax_list[j] * int8O_gemm_weight_amax_list[j]) / (
